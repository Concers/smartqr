    ...
    try {
      ...
      if (typeof destinationUrl === 'string' && destinationUrl.toLowerCase().startsWith('data:text/html')) {
        try {
          ...
          // Patch any "Rehbere Ekle" button (old alert or old downloadVCard) to use /api/vcard
          const rehbereButtonRe = /<button\s+onclick="[^"]*"[^>]*>([\s\S]*?Rehbere Ekle[\s\S]*?)<\/button>/i;
          if (rehbereButtonRe.test(htmlText)) {
            // Extract contact info directly from the HTML content
            ...
            // Find address: it's typically the rv after a MapPin icon or "Konum" label
            const konumIdx = htmlText.indexOf('Konum');
            if (konumIdx > -1) {
              const afterKonum = htmlText.slice(konumIdx);
              const addrM = afterKonum.match(/class="rv"[^>]*>([^<]+)/);
              if (addrM?.[1]) fields['address'] = addrM[1].trim();
            }
            // Find website and linkedin from href links
            const linkMatches = [...htmlText.matchAll(/href="https?:\/\/([^"]+)"/g)];
            for (const lm of linkMatches) {
              const url = lm[1];
              if (url.includes('linkedin.com')) fields['linkedin'] = url;
              else if (url.includes('maps.google')) continue;
              else if (url.includes('netqr.io')) continue;
              else if (!fields['website']) fields['website'] = url;
            }
            if (nmMatch?.[1]) fields['name'] = nmMatch[1].trim();
            if (ttMatch?.[1]) fields['title'] = ttMatch[1].trim();
            if (coMatch?.[1]) fields['company'] = coMatch[1].trim();
            if (telMatch?.[1]) fields['phone'] = telMatch[1].trim();
            if (mailMatch?.[1]) fields['email'] = mailMatch[1].trim();
            ...
            htmlText = htmlText.replace(
              rehbereButtonRe,
              `<a href="${vcardUrl}" style="${aStyle}">$1</a>`
            );
            // Remove any leftover downloadVCard script
      ...
      const buildVideoEmbedUrl = (rawUrl: string): string | null => {
        try {
