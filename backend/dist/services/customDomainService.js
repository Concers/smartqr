...
class CustomDomainService {
    constructor() {
        this.DNS_VERIFICATION_PREFIX = 'netqr-verification=';
    }
    async requestCustomDomain(userId, domain) {
        try {
    ...
    async verifyDNSOwnership(domain, token) {
        try {
            ...
            const hasValidRecord = txtRecords.some(record => record.includes(`${this.DNS_VERIFICATION_PREFIX}${verificationToken}`));
            if (hasValidRecord) {
    ...
    async approveDomain(domainId, adminId) {
        try {
    ...
    async rejectDomain(domainId, reason) {
        try {
            const domain = await prisma.customDomain.findUnique({
                where: { id: domainId },
                include: { user: true }
    ...
    async getUserCustomDomains(userId) {
        try {
    ...
    async getAllCustomDomains(status, page = 1, limit = 20) {
        try {
...
exports.default = CustomDomainService;
